...
A framework-agnostic validation engine (`ValidationEngine`)

From this engine, the library exposes two integrations:

An Express-specific integration (`Validator`)

A framework-agnostic integration for Node.js environments (`NodeValidator`)
...
It is important to note that `req-valid-express` does not rely on external libraries for runtime validation. The only dependency is inquirer, used exclusively by the CLI tool for interactive schema generation. This design decision aims to keep the library as simple as possible, reduce technical debt, and avoid unnecessary runtime dependencies.
