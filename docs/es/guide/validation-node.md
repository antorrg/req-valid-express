...
# Métodos principales de validación para Node

`req-valid-express` en su método `NodeValidator` expone tres métodos principales para validar datos de entrada:

[`validatePayload`](#validatebody)
- Alias: [`validateBody`](#validatebody)


[`validateOptions`](#validateheaders)

- Alias: `validateQuery`

[`validateId`](#validatequery)

- Alias: `paramId`

> Nota: Los alias se utilizan a fin de facilitar la comprensión y la implementación a desarrolladores que vengan de express, ya que la diferencia entre los métodos de Node y sus alias en express radica en la manera de ingresar y retornar la info.
